...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Beam search inference using vLLM"
    )
    parser.add_argument(
    ...
    if path.endswith('.jsonl'):
        with open(path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                # Expect each item to have a 'question' or 'prompt' field
...
def main():
    ...
    # Prepare sampling parameters for beam search
    ...
    print(f"Starting inference with beam size {args.beam_size} and batch size {args.batch_size}...")
