...
# ===== Configuration: No parameters needed, modify paths directly here =====
JSON_PATH    = r'test++.json'    # Path to dev.json
JSON3_PATH   = r'db_id.json'  # Path to third JSON file
DB_ROOT      = r'database'  # Database root directory
...
def main():
    ...
    # 2. Extract unique db_ids from json3
    unique_db_ids = get_unique_db_ids(JSON3_PATH)

    # 3. Filter safe items with non-empty last SQL execution, return with item_index
    safe_items = filter_safe_last_nonempty(dev_items, DB_ROOT)

    # 4. Split into in_list/out_list based on unique_db_ids
    in_list, out_list = [], []
    ...
    with open(os.path.join(OUTPUT_DIR, 'omni_gold_list.json'), 'w', encoding='utf-8') as f:
    ...
    # 6. Extract and save SQLs with difficulties
    ...
    # 7. Save original positions of gold_bird items in dev.json
