...
# Global process pool: Based on CPU count or custom size
...
def _worker(db_url: str, query: str) -> Tuple[str, Any, int]:
    """Execute SQL in subprocess and return (flag, DataFrame|None|Exception, ms)"""
    start = time.time()
    conn = None
    cur = None
    try:
...
def exec_on_db_proc(db_url: str, query: str, timeout_s: float = TIMEOUT_S) -> Tuple[str, Any, int]:
    """
    Submit task to global process pool and wait for timeout_s seconds:
- Normal: Return _worker output
- Timeout: Direct future.cancel(), return ("timeout", None, 0)
- Exception: Catch and return ("exception", Exception, 0)
    """
...
def _compute_metric(prediction: str, reference: str, db_url: str) -> dict:
    """
    Compare execution of single SQL prediction vs reference:
- Skip on timeout or exception (raise TimeoutError),
- Otherwise return {"results": 1 or 0}.
    """
    count = 0
    ...
    return {"results": int(correct),'empty':count}
...
def compute_evaluation_accuracy(
    ...
    """
    Evaluate overall execution accuracy for multiple SQLs:
- Skip timeout/exception entries,
- Return {"results": [...], "skipped": n, "execution_accuracy": float}
    ...
    total = len(results)
    print(total)
    accuracy = sum(results) / (total-empty_count) if total > 0 else 0.0
    logging.info(f"Evaluation done: non-skipped={total}, skipped={skipped}, accuracy={accuracy:.4f}")
    print("-----------------------------------------")
    print(empty_count)
    print("-----------------------------------------")
    return {"results": results, "skipped": skipped, "execution_accuracy": accuracy, 'empty_count':empty_count}